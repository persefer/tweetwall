        ...
        this.twitter = {
          ...
          updateTimestamps: function () {
            ...
            $(twitter.container).find('span.time').each(function () {
			  ...
			  // alert(twitter.query); -> markafoni OR trendyol OR limango OR Morhipo OR vipdukkan OR olmakistiyorum
            });
          },
...
//alert(this.settings.service); -> false
...
//	alert(this.settings.rpp); -> undefined		  
...
//			alert(protocol + '//' + baseURL + endpoint + '.json?' + queryString + '&callback=?');
...
//            return protocol + '//' + baseURL + endpoint + '.json?' + queryString + '&callback=?';
          ...
          parseTweet: function (json) {
            var tweet = {
/// TWITTER JSON DAN GELENLERI BURADA PARÇALIYORUZ
              id:         (json.id_str) ? json.id_str : json.id,
              text:       json.text,
              created_at: json.created_at,
//			  iso_language_code: json.iso_language_code,
//			  result_type:	json.metadata.result_type,
          ...
          renderTweet: function (tweet) {
            ...
            if (this.settings.showAuthor) {
              ...
              html += '<p class="text"><a href="' + tweet.profile_url + '" rel="external">' + tweet.from_user_name + '</a>: ';
            } else {
              html += '<p class="text"> ';
            }
...
/* ZAMANI SİMDİLİK BASMAYALIM. TWIT BOX KUTULARINDAN DISARI TASABILIYOR. CSS I HER EKRAN COZUNURLUGU ICIN AYARLAYIP ONDAN SONRA KOYMAK LAZIM
...
/*
// DEBUG
// a.php dosyasına neleri post ediyorum önce burada bir kontrol yapabilirim.
				  alert(tweet.completed_in + '\n' + 
						...
						tweet.result_type );
//
					$.post("a.php", {	tweet_id: tweet.id,
										from_user: tweet.from_user,
										from_user_name: tweet.from_user_name,
										profile_image_url: tweet.profile_image_url,
										text: tweet.text,
										date: tweet.created_at, result_type: tweet.result_type } );
*/
